...
public class DepartamentoController {
    ...
    @GetMapping("/{id}")
    @ApiOperation(value = "Obtiene una lista de todos los Departamento", response = DepartamentoDTO.class, responseContainer = "List", tags = "Departamentos")
    ...
    @PutMapping("/login")
    @ResponseBody
    @ApiOperation(value = "Inicio de sesión para conseguir un token de acceso", response = DepartamentoDTO.class, tags = "Seguridad")
    public ResponseEntity<?> login(@PathVariable(value = "estado") boolean estado) {
        try {
            Departamento departamento = new Departamento();
            departamento.isEstado();
            Optional<Departamento> departamentoFound = departamentoService.login(departamento);
            if (departamentoFound.isPresent()) {
                DepartamentoDTO departamentoDto = MapperUtils.DtoFromEntity(departamentoFound.get(), DepartamentoDTO.class);
                return new ResponseEntity<>(departamentoDto, HttpStatus.OK);
    ...
    @PostMapping("/")
    @ResponseBody
    @ApiOperation(value = "Inicio de sesión para conseguir un token de acceso", response = DepartamentoDTO.class, responseContainer = "List", tags = "Departamentos")
    ...
    public ResponseEntity<?> update(@PathVariable(value = "id") Long id, @RequestBody Departamento DepartamentoModified) {
        try {
            Optional<Departamento> departamentoUpdated = departamentoService.update(DepartamentoModified, true);
